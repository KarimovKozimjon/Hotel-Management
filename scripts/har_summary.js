...
function summarizeHar(har, label, { filterSubstring } = {}) {
  ...
  const rowsFiltered = filterSubstring
    ? rows.filter((r) => typeof r.url === 'string' && r.url.includes(filterSubstring))
    : rows;
  ...
  const notif = rows.filter((r) => typeof r.url === 'string' && r.url.includes('/api/notifications'))
...
function main() {
  ...
  if (!filePath) {
    console.error('Usage: node scripts/har_summary.js <path-to-har> [label]');
    process.exit(2);
  }
